  ...
  return (
    ...
    >
      ...
      <div className="container-x">
          ...
          {PILLARS.map((pillar, idx) => {
            ...
            return (
              <motion.div
                ...
                className="group relative overflow-hidden rounded-2xl border border-white/10 bg-white/5 p-6 backdrop-blur transition-all duration-300 hover:-translate-y-1 hover:border-secondary/40 hover:bg-white/10"
              >
                <div className="inline-flex h-14 w-14 items-center justify-center rounded-2xl bg-gradient-to-br from-secondary to-accent text-white shadow-glow-md transition-transform duration-300 group-hover:scale-110">
                ...
                <div
                  className="absolute -bottom-10 -right-10 h-32 w-32 rounded-full bg-secondary/20 blur-3xl opacity-0 transition-opacity duration-500 group-hover:opacity-100"
                  aria-hidden="true"
                />
              </motion.div>
            );
        ...
        >
          {STATS.map((s) => (
            <div key={s.label} className="bg-white/[0.03] px-6 py-7 text-center">
