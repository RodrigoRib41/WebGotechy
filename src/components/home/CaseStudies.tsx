...
export function CaseStudies() {
  ...
  const items = rawProjects.slice(0, 3).map((p) => localizeProject(p, isEn));

  if (!loading && items.length === 0) return null;

  return (
    <section className="section-dark overflow-hidden" aria-labelledby="cases-title">
      {/* Decoraciones — iluminaciones circulares brandbook */}
      ...
      {/* Flecha ornamental brandbook */}
      <div className="pointer-events-none absolute -left-12 top-1/3 opacity-[0.05]" aria-hidden="true">
        <ArrowMark size={280} outline color="#00F3FF" strokeWidth={3} />
      </div>
      ...
      <div className="container-x relative">
        ...
        >
          <div className="max-w-2xl">
            <span className="eyebrow-dark">{t('home.cases.eyebrow')}</span>
            <h2 id="cases-title" className="h2-display mt-5 text-white">
              {/* Brandbook: títulos solo blanco o celeste */}
              {t('home.cases.titleStart')}{' '}
              <span className="text-secondary">{t('home.cases.titleHighlight')}</span>
            </h2>
          </div>
          <Link
            to="/proyectos"
